    ...
    <div className="fixed inset-0 z-[500] bg-slate-950 flex flex-col overflow-hidden animate-in fade-in duration-300">
      ...
      <div className="bg-[#0f172a] px-10 py-8 text-white relative shrink-0 border-b border-white/5">
        ...
        <div className="flex flex-col gap-1 mb-8">
            <h2 className="text-xs font-black text-blue-500 uppercase tracking-[0.4em] ml-1">
                {isPrivileged ? 'Lead Record' : 'Lead Profile'}
            </h2>
            <h1 className="text-6xl font-black text-white uppercase tracking-tighter leading-tight">LD - {lead.lead_id.toString().padStart(6, '0')}</h1>
        </div>
      ...
      {/* 2. Main Content (Intelligent Role-Based Layout) */}
      <div className="flex-1 flex overflow-hidden bg-white">
         
         {['SALES_MANAGER', 'SALES_EXECUTIVE', 'RND_MANAGER', 'PACKAGING_MANAGER', 'PROJECT_MANAGER'].includes(userProfile?.role) ? (
             /* SPECIALIZED OPERATIONAL VIEW */
             <div className="flex-1 flex overflow-hidden">
                {/* LEFT: Technical Information Matrix */}
                <div className="w-[60%] border-r border-slate-100 overflow-y-auto scrollbar-hide p-12 space-y-12 bg-slate-50/30">
                    <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-sm">
                        <div className="flex items-center gap-3 mb-8">
                            <FlaskIcon className="text-indigo-600" size={24} />
                            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Scientific Formula</h4>
                        </div>
                        <div className="p-8 bg-slate-50/50 rounded-[32px] border border-slate-50 min-h-[140px] flex items-center">
                            <p className="text-sm text-slate-700 font-semibold leading-relaxed">
                                {lead.formulation_details || 'Regulatory and technical specifications pending submission.'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-sm">
                        ...
                        <div className="space-y-6">
                            <p className="text-sm text-slate-800 font-black italic bg-emerald-50/50 p-8 rounded-[32px] border border-emerald-100 leading-relaxed">
                                "{lead.requirement_details}"
                            </p>
                            <div className="flex items-center gap-4 px-8 py-5 bg-slate-50 rounded-3xl border border-slate-100">
                                <PackageIcon size={16} className="text-slate-400" />
                                <span className="text-xs font-bold text-slate-600 uppercase tracking-widest">
                                    {lead.packaging_details || 'Standard Institutional Packaging'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-sm">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-3">
                                <PaperclipIcon className="text-blue-600" size={24} />
                        ...
                        <div className="rounded-[32px] overflow-hidden border border-slate-100 shadow-xl">
                            {lead.requirement_brief ? (
                                isImage(lead.requirement_brief) ? (
                                    <img src={lead.requirement_brief} alt="Monitor" className="w-full h-[500px] object-cover" />
                                ) : (
                                    <div className="p-12 flex items-center gap-8 bg-slate-50">
                                        <div className="w-24 h-24 bg-white rounded-[40px] flex items-center justify-center border border-slate-200 shadow-2xl"><PaperclipIcon size={40} className="text-blue-600" /></div>
                                        <span className="font-bold text-2xl text-slate-900 break-all underline decoration-blue-500 decoration-8 underline-offset-8">{lead.requirement_brief}</span>
                                    </div>
                                )
                            ) : (
                                <div className="h-80 flex flex-col items-center justify-center bg-slate-50 border-4 border-dashed border-slate-100 m-8 rounded-[40px] opacity-30">
                                    <PackageIcon size={64} className="text-slate-300 mb-6" />
                                    <p className="text-xs font-black text-slate-400 uppercase tracking-[0.3em]">No Visual Assets Attached</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {/* RIGHT: Action Reporting & Dispatch */}
                <div className="flex-1 p-12 overflow-y-auto scrollbar-hide flex flex-col">
                    <div className="bg-slate-900 rounded-[48px] p-10 shadow-2xl shadow-slate-900/40 border border-white/5 space-y-8 sticky top-0">
                        ...
                        <textarea 
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-[32px] p-8 text-white font-bold text-base min-h-[260px] outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500/50 transition-all resize-none"
                            placeholder="Mandatory: Report findings or confirm task completion to advance the pipeline..."
                        />

                        <div className="pt-4 border-t border-white/5">
                            {['RND_MANAGER', 'PROJECT_MANAGER'].includes(userProfile?.role) ? (
                    ...
                    {(isFollowUp && (userProfile?.role === 'ADMIN' || userProfile?.role === 'OWNER')) && (
                        <div className="mt-10 p-10 bg-rose-50 border border-rose-100 rounded-[40px] flex flex-col gap-6">
                            <div className="space-y-1">
                                <h5 className="text-rose-900 font-extrabold text-lg flex items-center gap-2">
                                    <AlertCircle size={20} /> Admin Corrective Required
                                </h5>
                                <p className="text-rose-700 text-[11px] font-bold uppercase tracking-tight opacity-70 italic">Lead was rejected at Project Management gate.</p>
                            </div>
                            <button 
                                onClick={() => window.confirm('Reprocess this ticket back to R&D for technical rework?') && onAction?.('REPROCESS', 'Admin corrected issues. Resubmitting to R&D.')}
                                className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition-colors flex items-center justify-center gap-3 shadow-xl"
                            >
                                <RotateCcwIcon size={16} /> Reprocess Ticket to R&D
                            </button>
                        </div>
                    )}
                </div>
             ...
             /* ORIGINAL ADMINISTRATIVE OVERSIGHT VIEW */
             <>
                ...
                <div className="flex-1 flex flex-col p-10 space-y-10 overflow-y-auto scrollbar-hide">
                    ...
                    {isMyTurn ? (
                        <div className="bg-slate-900 rounded-[48px] p-8 mt-10 border border-slate-800 shadow-2xl flex flex-col space-y-6">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                        ...
                        <div className="mt-8 bg-rose-50 border border-rose-200 p-8 rounded-[32px] flex items-center justify-between">
                            ...
                            <button 
                                onClick={() => window.confirm('Are you sure you want to reprocess this ticket back into the R&D Queue?') && onAction?.('REPROCESS', 'Admin corrected ticket issues. Manually resubmitting into R&D for rework.')}
